...
public class PaoPaoExplode extends ElementObj {
	...
	public void showElement(Graphics g) {
		...
		g.drawImage(imgCenter.getImage(), getX(), getY(), getX() + 48, getY() + 48, 912, 623, 1000, 712, null);
		g.drawImage(imgUp.getImage(), getX(), getY() - 48 * power, getX() + 48, getY(), 891, 0, 1014,
				88 + 100 * (power - 1), null);
		g.drawImage(imgDown.getImage(), getX(), getY() + 48, getX() + 48, getY() + 48 + 48 * power, 891,
				1331 - 100 * (power - 1), 1022, 1411, null);
		g.drawImage(imgLeft.getImage(), getX() - 48 * power, getY(), getX(), getY() + 48, 6, 603,
				124 + 100 * (power - 1), 732, null);
		g.drawImage(imgRight.getImage(), getX() + 48, getY(), getX() + 48 + 48 * power, getY() + 48,
				1760 - 100 * (power - 1), 609, 1880, 731, null);
	...
	public boolean collide(ElementObj obj) {
		...
		Rectangle explodeColumn = new Rectangle(getX(), getY() - 48 * power, 48, 48 * (2 * power + 1));// 水泡爆炸十字纵向
		Rectangle explodeRow = new Rectangle(getX() - 48 * power, getY(), 48 * (2 * power + 1), 48);// 水泡爆炸十字横向
